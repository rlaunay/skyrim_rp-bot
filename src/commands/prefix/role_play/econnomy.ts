...
const econnmy: PrefixCommand = {
  ...
  async execute (message, args) {
    ...
    if (args[0] === 'add') {
      ...
      const giver = await messageSelectChar(message, message.author, 'Séléctionner le personnage qui va donné ');
      if (!giver) return;
  
      const receiver = await messageSelectChar(message, user, 'Séléctionner le personnage qui va recevoir ');
      if (!receiver) return;
  
      const choice = await messageConfirmation(message, `Voulez vous vraiment donner ${money} <:septims:${septims}> à \`${receiver.name}\` ?`);
      if (choice) {
